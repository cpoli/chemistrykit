...
:func:`chemistrykit.photochem.systems.stern_volmer.fit_stern_volmer` fits
a straight line to intensity-ratio-vs-quencher-concentration data -- the
same small numerical routine as
:mod:`chemistrykit.kinetics.utils.linear_regression` (used there for the
analogous Arrhenius-plot fit), reimplemented here rather than imported
cross-domain so that each chemistrykit domain subpackage depends only on
the shared :mod:`chemistrykit.constants` / :mod:`chemistrykit.integrators`
infrastructure, not on its sibling domains.
"""
