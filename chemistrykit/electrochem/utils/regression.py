...
:func:`chemistrykit.electrochem.systems.butler_volmer.fit_tafel_plot` fits
a straight line to the linearized Tafel equation (:math:`\\eta` vs.
:math:`\\log_{10}|i|`) and reports a goodness-of-fit statistic -- the same
small numerical routine as :mod:`chemistrykit.kinetics.utils.linear_regression`
(used there for the analogous Arrhenius-plot fit), reimplemented here
rather than imported cross-domain so that each chemistrykit domain
subpackage depends only on the shared :mod:`chemistrykit.constants` /
